...
export const FolderContent = ({ folder }: Props) => {
  ...
  const moveFolderIfOverDropzone = async () => {
  ...
  useEventListener('pointerup', moveFolderIfOverDropzone)
  ...
  const updateDraggedTypebotPosition = (e: MouseEvent) => {
  ...
  useEventListener('pointermove', updateDraggedTypebotPosition)
            ...
            {typebots &&
              typebots.map((typebot) => (
                <TypebotButton
                  ...
                  onPointerDown={triggerDrag(typebot)}
                />
              ))}
          </Wrap>
        </Stack>
      ...
      {draggedTypebot && (
        <Portal>
          <TypebotCardOverlay
            ...
            onMouseUp={moveFolderIfOverDropzone}
            pos="fixed"
            top="0"
            left="0"
            style={{
