...
export const Graph = ({
  ...
  const { setGraphPosition: setGlobalGraphPosition, connectingIds } = useGraph()
  ...
  const stopRightClickPropagation = (e: MouseEvent) => {
  ...
  const autoMoveGraphIfNecessary = (e: MouseEvent) => {
    if (!connectingIds)
  ...
  useEventListener('pointerdown', stopRightClickPropagation, undefined, {
    capture: true,
  })
  useEventListener('pointerup', dropBlock, graphContainerRef.current)
  useEventListener('pointermove', autoMoveGraphIfNecessary)
