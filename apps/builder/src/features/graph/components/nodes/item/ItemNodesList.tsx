...
export const ItemNodesList = ({
  ...
  const updateDraggedItemPosition = (event: MouseEvent) => {
  ...
  useEventListener('pointermove', updateDraggedItemPosition)
  useEventListener(
    'pointermove',
    expandNearestPlaceholder,
    mouseOverBlock?.element
  )
  useEventListener('pointerup', dropItem, mouseOverBlock?.element, {
    capture: true,
  })
