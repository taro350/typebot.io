...
export const BlockNodesList = ({
  ...
  const setBlockPosition = (event: MouseEvent) => {
  ...
  const expandNearestPlaceholder = (event: MouseEvent) => {
  ...
  useEventListener('pointermove', setBlockPosition)
  useEventListener('pointermove', expandNearestPlaceholder, groupRef.current)
  useEventListener('pointerup', dropNode, mouseOverGroup?.element, {
    capture: true,
  })
